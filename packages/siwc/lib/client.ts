...
export class SiwcMessage {
   ...
   */
  constructor(param: string | Partial<SiwcMessage>) {
   ...
   */
  toMessage(): string {
    const header = `${this.domain} wants you to sign in with your Conflux account:`;
  ...
  async validate(
    ...
    return new Promise<SiwcMessage>(async (resolve, reject) => {
      ...
      try {
        ...
        const parsedMessage = new SiwcMessage(message);
 ...
 * SiwcMessage in order to prevent replay attacks.
 *
